...
class ServiceScheduler(AsyncIOScheduler, Singleton):
    def __init__(self, *args, **kwargs):
        # Singleton(self).__init__(*args, **kwargs)
        super(ServiceScheduler, self).__init__(*args, **kwargs)
        job_store = MongoDBJobStore(
            database=settings.SCHEDULER.MONGO_DB_NAME,
            collection=settings.SCHEDULER.MONGO_COLLECTION_NAME,
            client=MongoClient(settings.SCHEDULER.URL)
        )
