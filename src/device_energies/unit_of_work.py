...
class DeviceEnergyUnitOfWork(AbstractUnitOfWork[DeviceEnergy]):
    ...
    async def list(self, *args, **kwargs) -> PaginationSchema[DeviceEnergy]:
        limit = kwargs.pop('limit', 10)
        offset = kwargs.pop('offset', 0)
        devices, count = await self._device_energy_repository.list(limit=limit, offset=offset, *args, **kwargs)
        paginator = Paginator[DeviceEnergy](limit=limit, offset=offset, models_list=devices, count=count)
